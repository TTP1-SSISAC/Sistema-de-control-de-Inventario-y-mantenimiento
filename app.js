...
/* ---------------- Utilidades de mes (Kardex) ----------------
   Se usan en el filtro del Kardex para agrupar movimientos por mes
   calendario. Igual que el resto de los formateadores de fecha de
   este archivo, se evita pasar un texto "yyyy-MM-dd..." directo por
   new Date() salvo como último recurso de compatibilidad, para no
   sufrir el desfase de zona horaria.
------------------------------------------------------------------- */
...
// FIX (fecha de falla se mostraba un día antes): igual que fmtFechaKardex,
// si el valor ya viene como texto plano "yyyy-MM-dd" (el caso del nuevo
// campo "Fecha de reporte" de Reportar falla) se lee directamente, SIN
// pasar por new Date(), porque new Date("yyyy-MM-dd") lo interpreta como
// medianoche UTC y al convertirlo a la hora local de Perú (UTC-5) retrocede
// un día. Solo se cae a new Date() como respaldo para celdas de fecha
// reales del Sheet (vencimientos de documentos, etc.).
...
// Repuebla Sistema/Componente de cada fila de falla ya en pantalla,
// conservando la selección actual mientras siga siendo válida (mismo
// criterio que refreshAllMetodoSelects para "método de reparación").
refreshAllFallaSistemaSelects();
}

/* ---------------- Falla: filas dinámicas (sistema + componente + código + detalle) ----------------
   Permite reportar varias fallas de la misma unidad en un solo envío,
   igual criterio que las filas de Ingreso y de Repuestos/Reparaciones:
   cada fila es independiente, con su propio Sistema → Componente →
   Código (auto) → Detalle, y al guardar se envía una llamada addFalla
   por cada fila, compartiendo los datos comunes (Placa, Personal,
   Fecha, Kilometraje, N° Documento, Usuario).
------------------------------------------------------------------- */

function renumberFallaRows() {
document.querySelectorAll('#fallasList .falla-row').forEach((row, i) => {
const num = row.querySelector('.falla-row-num');
if (num) num.textContent = `Falla ${i + 1}`;
});
}

// Repuebla el <select> de Componente de una fila según el Sistema
// elegido, conservando el componente actual si sigue existiendo en la
// nueva lista (desiredComponente es opcional, para el caso de refresco
// tras recargar datos).
function populateFallaComponentes(selComponente, sistema, desiredComponente) {
const componentes = state.catalogoFallas.filter(c => c['SISTEMA'] === sistema);
selComponente.innerHTML = componentes.map(c =>
`<option value="${c['COMPONENTE']}" data-codigo="${c['CÓDIGO']}">${c['COMPONENTE']}</option>`
).join('');
if (desiredComponente && componentes.some(c => c['COMPONENTE'] === desiredComponente)) {
selComponente.value = desiredComponente;
}
}

function updateFallaCodigo(row) {
const selComponente = row.querySelector('.falla-componente');
const inpCodigo = row.querySelector('.falla-codigo');
const codigo = selComponente.selectedOptions[0] ? selComponente.selectedOptions[0].dataset.codigo || '' : '';
inpCodigo.value = codigo;
}

// Repuebla Sistema y Componente de TODAS las filas de falla visibles
// (se llama tras cada loadAll(), por si el catálogo de fallas cambió),
// conservando la selección de cada fila mientras siga siendo válida.
function refreshAllFallaSistemaSelects() {
document.querySelectorAll('#fallasList .falla-row').forEach(row => {
const selSistema = row.querySelector('.falla-sistema');
const selComponente = row.querySelector('.falla-componente');
const prevSistema = selSistema.value;
const prevComponente = selComponente.value;
...
if (prevSistema && sistemas.includes(prevSistema)) selSistema.value = prevSistema;

populateFallaComponentes(selComponente, selSistema.value, prevComponente);
updateFallaCodigo(row);
});
}

// Agrega una nueva fila de falla al formulario.
function addFallaRow() {
const tpl = document.getElementById('fallaRowTemplate').content.cloneNode(true);
const row = tpl.querySelector('.falla-row');
document.getElementById('fallasList').appendChild(row);
renumberFallaRows();
...
const sistemas = [...new Set(state.catalogoFallas.map(c => c['SISTEMA']))];
selSistema.innerHTML = sistemas.map(s => `<option value="${s}">${s}</option>`).join('');
populateFallaComponentes(selComponente, selSistema.value);
updateFallaCodigo(row);
...
selComponente.addEventListener('change', () => updateFallaCodigo(row));

row.querySelector('.btn-remove-falla').addEventListener('click', () => {
row.remove();
renumberFallaRows();
// Siempre debe quedar al menos una fila disponible para cargar.
if (!document.querySelector('#fallasList .falla-row')) addFallaRow();
});
}

document.getElementById('btnAddFalla').addEventListener('click', () => addFallaRow());
addFallaRow(); // primera fila disponible al cargar la página
...
// ---- Lugar / Taller (hoja MT_LUGAR, columna "Taller") ----
renderLugarSelect();
...
// Repuebla el desplegable con búsqueda (Tom Select) de "Lugar" con los
// talleres de la hoja MT_LUGAR (columna "Taller"). Conserva la selección
// actual si el taller sigue existiendo en el catálogo; si no hay ninguna
// selección previa, precarga "SSI" por defecto cuando existe (mismo valor
// que traía el campo de texto antes de este cambio).
function renderLugarSelect() {
const sel = document.getElementById('repLugar');
if (!sel) return;
const tsPrevio = _tsMap.get(sel);
const valorPrevio = tsPrevio ? tsPrevio.getValue() : sel.value;

const opts = '<option value="" disabled selected>— Selecciona taller —</option>' +
state.lugares.map(l => `<option value="${l['Taller']}">${l['Taller']}</option>`).join('');
_initTomSelect(sel, opts);

const ts = _tsMap.get(sel);
if (valorPrevio && state.lugares.some(l => l['Taller'] === valorPrevio)) {
ts.setValue(valorPrevio, true);
} else if (!valorPrevio && state.lugares.some(l => l['Taller'] === 'SSI')) {
ts.setValue('SSI', true);
}
}

// Agrega un nuevo taller al catálogo (hoja MT_LUGAR, vía backend, acción
// 'addLugar') y lo deja seleccionado en el formulario.
document.getElementById('btnAddLugar').addEventListener('click', async () => {
const val = window.prompt('Nombre del nuevo taller:');
if (!val || !val.trim()) return;
const nombre = val.trim();
try {
await apiPost('addLugar', { 'Taller': nombre });
toast('"' + nombre + '" agregado al catálogo de talleres', 'ok');
await loadAll();
const sel = document.getElementById('repLugar');
const ts = _tsMap.get(sel);
if (ts) ts.setValue(nombre, true);
} catch (err) {
toast('Error: ' + err.message, 'error');
}
});
...
// NUEVO: fecha de reporte de la falla. Se precarga con la fecha actual
// (texto plano "yyyy-MM-dd", mismo criterio del resto de la app: nunca
// se pasa por new Date() al mostrarla, para evitar el desfase de zona
// horaria), pero el usuario puede editarla libremente antes de guardar
// (por ejemplo, para reportar una falla ocurrida un día anterior).
const inpFallaFecha = document.getElementById('fallaFecha');

function setFechaFallaAhora() {
if (!inpFallaFecha) return;
const now = new Date();
const pad = n => String(n).padStart(2, '0');
inpFallaFecha.value = `${now.getFullYear()}-${pad(now.getMonth()+1)}-${pad(now.getDate())}`;
}
setFechaFallaAhora();
...
const filas = [...document.querySelectorAll('#fallasList .falla-row')].map(row => ({
sistema: row.querySelector('.falla-sistema').value,
componente: row.querySelector('.falla-componente').value,
codigo: row.querySelector('.falla-codigo').value,
detalle: row.querySelector('.falla-detalle').value.trim()
})).filter(f => f.sistema && f.componente && f.detalle);

if (!filas.length) {
toast('Agrega al menos una falla con su detalle', 'error');
return;
}

// Campos comunes del reporte (Placa, Personal, Fecha, Kilometraje,
// N° Documento, Usuario): son los únicos con atributo "name" que quedan
// en el formulario, así que formToObject() ya no trae Sistema/Componente.
const comunes = formToObject(e.target);
...
// Una llamada addFalla por cada fila, en orden, igual criterio que la
// factura de Ingreso con varios productos.
for (const f of filas) {
const data = Object.assign({}, comunes, {
'SISTEMA': f.sistema,
'COMPONENTE': f.componente,
'CÓDIGO': f.codigo,
'DETALLE/OCURRENCIA': f.detalle
});
try {
await apiPost('addFalla', data);
ok++;
} catch (err) {
errores.push(`${f.sistema} — ${f.componente}: ${err.message}`);
}
}
...
if (errores.length) {
toast(`${ok} falla(s) registrada(s). Con errores: ${errores.join(' | ')}`, ok ? '' : 'error');
} else {
toast(`Falla${ok > 1 ? 's' : ''} reportada${ok > 1 ? 's' : ''} correctamente (${ok})`, 'ok');
}

e.target.reset();
document.getElementById('fallasList').innerHTML = '';
addFallaRow();
setFechaFallaAhora();
await loadAll();
